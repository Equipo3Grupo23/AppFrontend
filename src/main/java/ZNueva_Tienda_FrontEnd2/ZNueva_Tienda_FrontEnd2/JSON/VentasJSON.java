...
public class VentasJSON {
	...
	public static ArrayList<Ventas> parsingVentas(String json)
		...
		while (i.hasNext()) {
			...
			venta.setCodigo_venta(innerObj.get("codigo_venta").toString());
			venta.setCedula_cliente(innerObj.get("cedula_cliente").toString());
			venta.setCedula_usuario(innerObj.get("cedula_usuario").toString());
			venta.setIva_venta(innerObj.get("iva_venta").toString());
			venta.setValor_venta(innerObj.get("valor_venta").toString());
			venta.setTotal_venta(innerObj.get("total_venta").toString());
			lista.add(venta);
		}
	...
	public static int postJSON(Ventas venta) throws IOException {
		...
		String data = "{" + "\"codigo_venta\":\"" + venta.getCodigo_venta() + "\",\"cedula_cliente\": \""
				+ venta.getCedula_cliente() + "\",\"cedula_usuario\": \"" + venta.getCedula_usuario()
				+ "\",\"iva_venta\":\"" + venta.getIva_venta() + "\",\"total_venta\":\"" + venta.getTotal_venta() + "\",\"Valor_venta\":\"" + venta.getValor_venta() + "\"}";
	...
	public static ArrayList<Ventas> getJSONauth() throws IOException, ParseException,
	org.apache.tomcat.util.json.ParseException, org.json.simple.parser.ParseException {

url = new URL(sitio + "ventas/auth");
HttpURLConnection http = (HttpURLConnection) url.openConnection();
http.setRequestMethod("GET");
http.setRequestProperty("Accept", "application/json");
InputStream respuesta = http.getInputStream();
