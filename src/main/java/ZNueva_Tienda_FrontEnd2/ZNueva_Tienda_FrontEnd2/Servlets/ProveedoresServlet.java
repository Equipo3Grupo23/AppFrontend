package ZNueva_Tienda_FrontEnd2.ZNueva_Tienda_FrontEnd2.Servlets;

import java.io.IOException;
import java.io.PrintWriter;
import java.util.ArrayList;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import ZNueva_Tienda_FrontEnd2.ZNueva_Tienda_FrontEnd2.JSON.ProveedoresJSON;
import ZNueva_Tienda_FrontEnd2.ZNueva_Tienda_FrontEnd2.JSON.UsuariosJSON;
import ZNueva_Tienda_FrontEnd2.ZNueva_Tienda_FrontEnd2VO.Proveedores;
import ZNueva_Tienda_FrontEnd2.ZNueva_Tienda_FrontEnd2VO.Usuarios;

/**
 * Servlet implementation class ProveedoresServlet
 */
@WebServlet("/ProveedoresServlet")
public class ProveedoresServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProveedoresServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		String tipo = request.getParameter("tipo");

		if ("listar".equals(tipo)) {
			this.listarProveedores(request, response);
		}
			if ("buscar".equals(tipo)) {
				this.buscarProveedor(request, response);
			}
			/*if ("borrar".equals(tipo)) {
				this.borrarproveedor(request, response);
			}*/
		 if ("agregar".equals(tipo)) {
			this.agregarProveedor(request, response);
		}

	}
	

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

	public void agregarProveedor(HttpServletRequest request, HttpServletResponse response) {

		Proveedores proveedores = new Proveedores();

		proveedores.setNombre_proveedor(request.getParameter("nombre"));
		proveedores.setNit_proveedor(request.getParameter("nit"));
		proveedores.setTelefono_proveedor(request.getParameter("telefono"));
		proveedores.setDireccion_proveedor(request.getParameter("direccion"));
		proveedores.setCiudad_proveedor(request.getParameter("ciudad"));
		int respuesta = 0;

		try {
			respuesta = ProveedoresJSON.postJSON(proveedores);
			PrintWriter writer = response.getWriter();
			if (respuesta == 200) {
				writer.println("Registro agregado");
			} else {
				writer.println("Error" + respuesta);
			}
			writer.close();
		} catch (IOException e) {
			e.printStackTrace();
		}

	}
	
	//FUNCIONANDO NO TOCAR
		public void listarProveedores(HttpServletRequest request, HttpServletResponse response) {
			try {
				ArrayList<Proveedores> lista = ProveedoresJSON.getJSON();
				String pagina = "/resultadoProveedores.jsp";
				request.setAttribute("lista", lista);
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(pagina);
				dispatcher.forward(request, response);

			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
			}
		}

		public void buscarProveedor(HttpServletRequest request, HttpServletResponse response)
				throws ServletException, IOException {
			try {
				ArrayList<Proveedores>listaFil = ProveedoresJSON.getJSON();
				request.setAttribute("listaFil", listaFil);
				System.out.println(listaFil);
				String nit = request.getParameter("txtnit");
				System.out.println(nit);
				Long nit_proveedor = Long.parseLong(nit);
				System.out.println(nit_proveedor);
				
				Proveedores resultado = null;
				for (Proveedores usr : listaFil) {
					System.out.println(usr);
					long nitlong = Long.parseLong(usr.getNit_proveedor());
					System.out.println(nitlong);
					if (nitlong == 0) {
						request.getRequestDispatcher("/principal.jsp").forward(request, response);
						System.out.println("No se encontraron datos");
						break;
				
					} else {
						if (nit_proveedor == nitlong) {
							resultado = usr;
							String nit2 = resultado.getNit_proveedor();
							String nombre2 = resultado.getNombre_proveedor();
							String telefono2 = resultado.getTelefono_proveedor();
							String direccion2 = resultado.getDireccion_proveedor();
							String ciudad2 = resultado.getCiudad_proveedor();
							request.setAttribute("nit2", nit2);
							request.setAttribute("nombre2", nombre2);
							request.setAttribute("telefono2", telefono2);
							request.setAttribute("direccion2", direccion2);
							request.setAttribute("ciudad2", ciudad2);
							String pagina = "/proveedorEncontrado.jsp";
							RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(pagina);
							dispatcher.forward(request, response);
							break;
						}
					}
				}
				request.setAttribute("nit2", "Proveedor no existe");
				request.setAttribute("nombre2", "");
				request.setAttribute("telefono2", "");
				request.setAttribute("direccion2", "");
				request.setAttribute("ciudad2", "");
				request.getRequestDispatcher("/proveedorEncontrado.jsp").forward(request, response);

			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}
	}
